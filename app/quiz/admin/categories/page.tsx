...
export default function AdminCategoriesPage() {
  ...
  async function load() {
    const [catRes, langRes] = await Promise.all([
      ...
      fetch("/api/quiz/admin/languages"),
    ])
  ...
  async function create() {
    ...
    setSaving(true)
    await fetch("/api/quiz/admin/categories", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: newName.trim(), languageId: newLanguageId ? parseInt(newLanguageId) : null }),
    })
    setNewName("")
    setNewLanguageId("")
    setSaving(false)
  ...
  async function update(id: number) {
    ...
    setSaving(true)
    await fetch(`/api/quiz/admin/categories/${id}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: editName.trim(), languageId: editLanguageId ? parseInt(editLanguageId) : null }),
    })
    setEditId(null)
    setSaving(false)
    load()
  ...
  async function remove(id: number) {
    ...
    await fetch(`/api/quiz/admin/categories/${id}`, { method: "DELETE" })
    load()
  }

  const selCls = "h-8 px-2 text-sm border border-slate-200 rounded outline-none focus:border-blue-400 bg-white"
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      <div className="flex gap-2 mb-6">
        ...
        <select value={newLanguageId} onChange={e => setNewLanguageId(e.target.value)} className="h-9 px-2 text-sm border border-slate-200 rounded-lg outline-none focus:border-blue-400 bg-white">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {categories.map(cat => {
                const langName = languages.find(l => l.id === cat.languageId)?.name ?? "—"
                return (
                  <tr key={cat.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      {editId === cat.id ? (
                        <select value={editLanguageId} onChange={e => setEditLanguageId(e.target.value)} className={selCls}>
                          <option value="">All Languages</option>
                          {languages.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
                        </select>
                      ) : (
                        <span className="text-slate-500">{langName}</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2 justify-end">
                        {editId === cat.id ? (
                          ...
                          <>
                            <button onClick={() => { setEditId(cat.id); setEditName(cat.name); setEditLanguageId(cat.languageId ? String(cat.languageId) : "") }} className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-md cursor-pointer"><Edit className="w-4 h-4" /></button>
                            <button onClick={() => remove(cat.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-md cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </td>
