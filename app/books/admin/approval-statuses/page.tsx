...
export default function AdminBookApprovalStatusesPage() {
  ...
  async function load() {
    const res = await fetch("/api/books/admin/approval-statuses")
  ...
  async function create() {
    ...
    setSaving(true)
    await fetch("/api/books/admin/approval-statuses", {
      method: "POST",
  ...
  async function remove(id: number, name: string) {
    ...
    await fetch(`/api/books/admin/approval-statuses/${id}`, { method: "DELETE" })
    load()
  }
