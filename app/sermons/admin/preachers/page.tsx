...
export default function AdminSermonPreachersPage() {
  ...
  async function load() {
    ...
    if (res.ok) setPreachers(await res.json())
    setLoading(false)
  }
  ...
  async function create() {
    ...
    setSaving(true)
    await fetch("/api/sermons/admin/preachers", {
      method: "POST",
  ...
  async function update(id: number) {
    ...
    setSaving(true)
    await fetch(`/api/sermons/admin/preachers/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function remove(id: number) {
    ...
    await fetch(`/api/sermons/admin/preachers/${id}`, { method: "DELETE" })
    load()
  }

  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Preachers</h1>
      <div className="flex gap-2 mb-6">
        <input
          value={newName}
          onChange={e => setNewName(e.target.value)}
          onKeyDown={e => e.key === "Enter" && create()}
          placeholder="New preacher name…"
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
                ...
                <tr key={p.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      {editId === p.id ? (
                        <>
                          <button onClick={() => update(p.id)} disabled={saving} className="px-3 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 cursor-pointer">Save</button>
              ...
              {preachers.length === 0 && (
                <tr><td colSpan={2} className="px-4 py-8 text-center text-slate-400 text-sm">No preachers</td></tr>
              )}
            </tbody>
          </table>
        </div>
