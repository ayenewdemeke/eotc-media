...
export default async function CollectionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  for (const cv of collection.verses) {
    ...
    const bookName = locale === "am"
      ? (v.book.amharicName ?? v.book.englishName)
      : (v.book.englishName ?? v.book.amharicName)
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                {collection.verses.length} {collection.verses.length !== 1 ? t("col_verse_pl") : t("col_verse_sg")}
              </p>
            </div>
          </div>
