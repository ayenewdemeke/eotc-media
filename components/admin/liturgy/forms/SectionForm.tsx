...
export function SectionForm({ initialData, inModal, onSuccess, onCancel }: SectionFormProps) {
  ...
  async function onSubmit(values: SectionFormValues) {
    ...
    try {
      const url = isEditing
        ? `/api/liturgy/admin/sections/${initialData.id}`
        : "/api/liturgy/admin/sections"
      ...
      if (onSuccess) {
        ...
        router.push("/liturgy/admin/sections")
        router.refresh()
      }
    } catch (error) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Name (Ge&apos;ez)</FormLabel>
              <FormControl>
                <Input
                  placeholder="Enter Ge'ez name"
                  {...field}
                  disabled={isLoading}
                />
              </FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  placeholder="e.g., Preface, Sanctus, Anaphora of the Apostles"
                  {...field}
                  disabled={isLoading}
                />
              </FormControl>
        ...
        <div className="flex gap-4">
          ...
          <Button
            type="button"
            variant="outline"
            onClick={() => onCancel ? onCancel() : router.push("/liturgy/admin/sections")}
            disabled={isLoading}
          >
            Cancel
          </Button>
