...
export default function SermonChannelInfiniteGrid({
  initialChannels,
  initialTotal,
  initialTotalPages,
}: Props) {
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/sermons/channels?page=${nextPage}`)
      if (!res.ok) return
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                {channel._count.sermons} {channel._count.sermons === 1 ? "sermon" : "sermons"}
              </p>
            </div>
          </Link>
        ))}
